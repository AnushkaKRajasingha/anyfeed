...
FEEDBLOCK;
    ...
    public function loadFeedItems()
    {
        if (isset($_POST['feedurl']) && !empty($_POST['feedurl'])) {
            try {
                ...
                $reader4  = new ScsFeedReader($options);
    ...
    function anyfeed_scripts()
    {
        ...
        wp_localize_script('anyfeed-scr', 'anyfeed_var', array(
            ...
            'feeditem' => '<div class="feed-item" data-timestamp="{{timestamp}}"> <div class="feed-image" style="background-image: url(' . $imageloadurl . ');"> </div> <div class="feed-content"> <h3 class="feed-title"><a href="{{link}}" class="feed-title">{{title}}</a></h3> <div class="feed-content"> <span class="pub-date">Date : {{date}} | From : {{from}}</span> <div class="item-desc">{{description}}</div> </div> <a href="{{link}}" class="feed-more">read more</a> </div> </div>'
        ));
